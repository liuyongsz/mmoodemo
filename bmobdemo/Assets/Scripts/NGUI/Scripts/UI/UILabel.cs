...
public class UILabel : UIWidget
{
    ...
    [HideInInspector]
    [SerializeField]
    public bool mbUseSnailFont = true;
	[HideInInspector][SerializeField] public Font mTrueTypeFont;
	[HideInInspector][SerializeField] UIFont mFont;
#if !UNITY_3_5
	[MultilineAttribute(6)]
#endif
	[HideInInspector][SerializeField] string mText = string.Empty;
    ...
    [HideInInspector][SerializeField] bool isChanged = false;
    [HideInInspector][SerializeField] public string mOriginalText = string.Empty;
    [HideInInspector][SerializeField] public bool mIntConvert = false;
    ...
    string mProcessedText = string.Empty;
    ...
    public string text
    {
        ...
        set
        {
            ...
            {
                originalText = string.Empty;      //清空字符转义符 YangDan
                mText = value;
                shouldBeProcessed = true;
                ProcessAndRequest();
            }
    ...
    static string[] separater = new string[] { "\\n" };
    static System.Text.StringBuilder sb = new System.Text.StringBuilder();
    /// <summary>
    /// 强制换行处理
    /// </summary>
    /// <param name="strContext">原文内容</param>
    /// <returns></returns>
    public static string ForceBR(string strContext)
    {
        ...
        string[] strLines = strContext.Split(separater, System.StringSplitOptions.None);
        if (strLines != null)
        {
            sb.Remove(0, sb.Length);
            int len = strLines.Length;
            for (int i = 0; i < len - 1; ++i)
            {
                sb.Append(strLines[i]);
                sb.Append("\n");
            }
            sb.Append(strLines[len - 1]);
            return sb.ToString();
        }
        return strContext;
    }
...
#endif
        if (bitmapFont == Config.NGUIFont)
        {
            bitmapFont = Config.SnailFont;
        }
        else if( bitmapFont != null )
        {
            ///是其他图集字体的话
            LoadFontUIAtlas(bitmapFont.name);
        }
    ...
    private void LoadFontUIAtlas(string strAtlasName)
    {
        if (Application.isPlaying)
        {
            string strFileName = "Local/Font/" + strAtlasName;
            ...
            {
                ///编辑器模式
                UnityEngine.Object oAsset = Resources.Load(strFileName);
                OnFileLoaded(oAsset);
            }
        }
#if UNITY_EDITOR
        ...
        {
            string strFileName = "Assets/Resources/Parts/UIAtlas/" + strAtlasName+".asset";
            UnityEngine.Object oAsset = UnityEditor.AssetDatabase.LoadMainAssetAtPath(strFileName) as UnityEngine.Object;
            OnFileLoaded(oAsset);
        }
#endif
    }
    ...
    {
        UIAtlasAsset atlasAsset = args[0] as UIAtlasAsset;
        if (atlasAsset == null)
            return;

        if ( bitmapFont != null && bitmapFont.atlas == null )
        {
#if UNITY_EDITOR
            AppendLinkObject(bitmapFont);
#endif
            bitmapFont.atlas = atlasAsset.uiAtlas;
        }
    }
#if UNITY_EDITOR
    /// <summary>
    /// 链接对象列表，图集，字体，图集字体
    /// </summary>
    static List<UnityEngine.Object> mLoadedObject = new List<UnityEngine.Object>();
    /// <summary>
    /// 添加链接对象
    /// </summary>
    /// <param name="atlas"></param>
    private static void AppendLinkObject(UnityEngine.Object atlas)
    {
        if (!Application.isPlaying)
            return;
    ...
    public static void ClearLinkObjects()
    {
        ...
        {
            UnityEngine.Object obj = mLoadedObject[i];
            if (obj == null)
                continue;

            if (obj is UIFont)
            {
                ((UIFont)obj).atlas = null;
            }
        }
        mLoadedObject.Clear();
    }
    ...
    /// </summary>
#if UNITY_4_6_9 || UNITY_4_7
    static void OnFontTextureChanged(Font font)
    {
    ...
    void ProcessText(bool legacyMode, bool full)
    {
        ...
        if (mPrintedSize > 0)
        {
            ...
            for (int ps = mPrintedSize; ps > 0; --ps)
            {
                ...
                if (mIntConvert)
                {
                    if (string.IsNullOrEmpty(mText) || string.IsNullOrEmpty(mText.Trim()))
                    {
                        fits = NGUIText.CustomWrapText(mText, out mProcessedText);
                    }
                    else
                    {

                        fits = NGUIText.CustomWrapText(ConvertNumber(Int32.Parse(mText, 0)), out mProcessedText);     //  修复ResizeHeight属性不换行功能
                    } 
                }
                else
                {
...
#else
                if (mIntConvert && intConvertFunc != null)
                {
                    if (string.IsNullOrEmpty(mText) || string.IsNullOrEmpty(mText.Trim()))
                    {
                        fits = NGUIText.CustomWrapText(mText, out mProcessedText);
                    ...
                    {
                        fits = NGUIText.CustomWrapText(intConvertFunc(mText), out mProcessedText);     //  修复ResizeHeight属性不换行功能
                    }
                }
                else
                {
    ...
    public static string ConvertNumber(long value)
    {
        ...
        {
            return StringBuilder((value / hundredmillion), GetUIString("UI230114"));
        }
        else if (value >= tenthousand)
        {
            return StringBuilder((value / tenthousand) + GetUIString("UI230113"));
        }
    ...
    /// 数字转换为 中文
    /// </summary>
    public static OnQueryString intConvertFunc = null;
    ...
    public void PrintOverlay(int start, int end, UIGeometry caret, UIGeometry highlight, Color caretColor, Color highlightColor)
    {
        ...
        if (highlight != null && start != end)
        {
            ...
            if (highlight.verts.size > startingVertices)
            {
                ...
                Color32 c ;
                c.r  = (byte)highlightColor.r;
                c.g = (byte)highlightColor.g;
                c.b = (byte)highlightColor.b;
                c.a = (byte)(highlightColor.a * alpha);
        ...
        cc.r = (byte)caretColor.r;
        cc.g = (byte)caretColor.g;
        cc.b = (byte)caretColor.b;
        cc.a = (byte)(caretColor.a * alpha);
    ...
    public void SetCustomProgress()
    {
        if (UIProgressBar.current != null)
        {
            float mValue = UIProgressBar.current.value;
            if (mValue <= .1f)
                mValue = .1f;
            else if (mValue >= .9)
                mValue = .9f;
            text = Mathf.RoundToInt(mValue * 100f) + string.Empty;
        }
    }

    /// <summary>
    /// 自定义百分比，当滚动条改变时调用(0-1)
    /// 添加人zhangrj   
    /// 日期 20141204
    /// </summary>
    public void SetCustomVoiceProgress()
    {
        if (UIProgressBar.current != null)
        {
            ...
            UIProgressBar.current.value = (float)System.Math.Round(mValue, 1);
            text = System.Math.Round(mValue, 1) * 100 + string.Empty;
        }
    }
    ...
    public void SetCustomScreenCount()
    {
        ...
        {
            float mValue = UIProgressBar.current.value;
            int count = Mathf.RoundToInt(mValue * 50f);
            if (count < 10)
                count = 10;
            else if (count > 50)
                count = 50;
