...
public class EnhanceScrollView : MonoBehaviour
{
    ...
    public delegate void OnDragEndCallBack(GameObject go);
    public OnDragEndCallBack callBack;
    ...
    public void UpdateEnhanceScrollView(float fValue)
    {
        for (int i = 0; i < listEnhanceItems.Count; i++)
        {
            ...
            float yValue = positionCurveY.Evaluate(depthCurveValue) * SpaceHeight;
            itemScript.UpdateScrollViewItems(xValue, depthCurveValue, depthFactor, listEnhanceItems.Count, yValue, scaleValue);
        }
    }
    ...
    {
        float evaluateValue = positionCurveY.Evaluate(sliderValue + added) * totalHorizontalWidth;
        return evaluateValue;
    }
    ...
    void RotateTrans(params object[] arg)
    {
        bool isRight = (bool)arg[0];
        if (isRight)
        {
            gearRotate.Rotate(6 * Vector3.forward);
        }
        else
        {
            gearRotate.Rotate(-6 * Vector3.forward);
    ...
    public void OnDragEnhanceViewMove(Vector2 delta)
    {
        ...
        if (Mathf.Abs(delta.y) > 0.0f)
        {
            ...
            if (gearRotate != null)
                gearRotate.Rotate(delta.y * Vector3.forward);
