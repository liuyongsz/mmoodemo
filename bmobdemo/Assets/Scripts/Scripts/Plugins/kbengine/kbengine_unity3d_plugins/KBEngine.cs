namespace KBEngine
{
	...
	*/
	...
	public class KBEngineApp
	{
		...
		void installEvents()
		{
			Event.registerIn(this, "createAccount");
			Event.registerIn(this, "login");
			Event.registerIn(this, "reLoginBaseapp");
			Event.registerIn(this, "resetPassword");
			Event.registerIn(this, "bindAccountEmail");
			Event.registerIn(this, "newPassword");
			
			// 内部事件
			Event.registerIn(this, "_closeNetwork");
		}
		...
		*/
        ...
        /// <param name="did">动态id</param>
        /// <returns></returns>
        public Entity GetEntity(int did)
        {
            Entity e;
            if (entities.TryGetValue(did, out e))
                return e;
        ...
        /// <summary>获取怪物</summary>
        public Monster GetMonster(int did)
        {
            Entity e;
            if (entities.TryGetValue(did, out e))
                return e as Monster;
        ...
        /// <summary>获取Avatar</summary>
        public KBEngine.Avatar GetAvatar(int did)
        {
            Entity e;
            if (entities.TryGetValue(did, out e))
                return e as KBEngine.Avatar; 

            return null;
        }

        /// <summary>获取Card</summary>
        public KBEngine.Card GetCard(int did)
        {
            Entity e;
            if (entities.TryGetValue(did, out e))
                return e as KBEngine.Card;
		...
		*/
		...
		{  
			if(noconnect)
			{
                ...
                {
                    baseappIP = "1667vz2198.iok.la";
                    ////外网配置
                    if (baseappPort == 20015)
                    {
                        baseappPort = 10863;
                    }
                }
			...
			while(stream.length() > 0)
			{
				...
				//if(setmethod != null)
				{
					if(propertydata.isBase())
					{
						...
						//if(entity.inited)
                        {
                            string methodName = "set_" + propertydata.name;
                            //Debug.LogError(entity.className       +"    "   + propertydata.name);
                            if (entity.className.Equals("Avatar"))
                            {
                                MethodInfo m = typeof(KBEngine.Avatar).GetMethod("processOutEvent");
                                object[] args = new object[] { entity,oldval };
                                m.Invoke(entity, new object[] { methodName, args });
                            }
                            else
                            {
		...
		*/
		...
		public void onRemoteMethodCall_(Int32 eid, MemoryStream stream)
		{
			...
			try
			{
                ...
                MethodInfo m = typeof(KBEngine.Avatar).GetMethod("processOutEvent");
                methoddata.handler = m;

                object[] param = new object[] {methoddata.name, args};
                methoddata.handler.Invoke(entity, param);
		...
		*/
        ...
        public void CreateClientEntity(int eid,int uentityType,sbyte isOnGround = 1)
        {
            if (entity_id > 0 && entity_id != eid)
                _entityIDAliasIDList.Add(eid);

            string entityType = uentityType == 1 ? "Card" : "Monster";

            Entity entity = null;

            if (!entities.TryGetValue(eid, out entity))
            {
                Type runclass = uentityType == 1 ? typeof(KBEngine.Card) : typeof(KBEngine.Monster);
                entity = (Entity)Activator.CreateInstance(runclass);
                entity.id = eid;
                entity.className = entityType;
