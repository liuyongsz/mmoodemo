...
public class SymbolInput : UIInput
{
    ...
    //    static protected string mLastIME = string.Empty;
    //#endif
    ...
    protected new void Init()
    {
        if (mDoInit && newLabel != null)
        {
            ...
            if (newLabel.labelText.alignment == NGUIText.Alignment.Justified)
            {
                ...
                LogSystem.LogWarning("Input fields using labels with justified alignment are not supported at this time", this);
            }
    ...
    protected new void OnSelectEvent()
    {
        ...
        if (mDoInit) Init();
		LogSystem.LogWarning("keyBoard open 1");
        if (newLabel != null && NGUITools.GetActive(this))
        {
            ...
            {
#if UNITY_IPHONE
				IOSKeyBoard.isPassword = (inputType == InputType.Password)?true:false;
				
				IOSKeyBoard.mKeyIn = newLabel.text;
				
				IOSKeyBoard.mKeyOut = newLabel.text;
				
				IOSKeyBoard.mKeyboard = mKeyboard;
...
#endif
            {
                ...
                pos.y = ResolutionConstrain.Instance.height - pos.y;
...
#if UNITY_IPHONE
				    IOSKeyBoard.mKeyboard = mKeyboard;
				    IOSKeyBoard.U3DtouchToHideKeyBoard();
#endif
                }
			}
#endif
    ...
    protected new string GetLeftText()
    {
        ...
        if( string.IsNullOrEmpty(mValue) || min < 0)
            return string.Empty;

        if (min >= mValue.Length)
        {
            return mValue;
        }

        return mValue.Substring(0, min);
    }
    ...
    protected new string GetRightText()
    {
        ...
        return (string.IsNullOrEmpty(mValue) || max >= mValue.Length) ? string.Empty : mValue.Substring(max);
    }
    ...
    public new void UpdateLabel()
    {
        if (newLabel != null)
        {
            ...
            if (isEmpty)
            {
                processed = selected ? string.Empty : mDefaultText;
                RestoreLabelPivot();
            }
            else
            {
                ...
                //selected = processed.Length == cursorPosition;

                // Start with text leading up to the selection
                //int selPos = selected ? Mathf.Min(processed.Length, cursorPosition) : 0;  //修改聊天 添加内容时，不更新问题 2015 4 20
                int selPos = Mathf.Min(processed.Length, cursorPosition);
                ...
                //if (selected && newLabel.labelText.overflowMethod == UILabel.Overflow.ClampContent)   //修改聊天 添加内容时，不更新问题 2015 4 20
                if (newLabel.labelText.overflowMethod == UILabel.Overflow.ClampContent)
                {
