...
public class UtilTools
{
    ...
    static string[] IntStrings = new string[]
   {
    ...
    /// 判断是不是自己好友
    /// </summary>
    public static bool isMyFriend(int dbid)
    {
        for (int i = 0; i < FriendMediator.friendList.Count; ++i)
        {
            if (FriendMediator.friendList[i].dbid == dbid)
                return true;
        }
        return false;
    }
    ...
    public static T GetParent<T>(RectTransform trans)
    {
        T obj = default(T);
    ...
    public static void SetStar(int starLevel, UISprite[] star, int maxStar = 6)
    {
        for (int i = 0; i < star.Length; ++i)
        {
            star[i].gameObject.SetActive(maxStar - 1 >= i);
            if (starLevel > i)
            {
                star[i].spriteName = "star2";
            }
            else
            {
                star[i].spriteName = "star1";
            }
        }
    }
    ...
    public static void SetSkillType(int type,UILabel label)
    {
        ...
        {
            case 1:
                label.text = TextManager.GetUIString("zhudong");
                break;
            case 2:
                label.text = TextManager.GetUIString("beidong");
                break;
            case 3:
                label.text = TextManager.GetUIString("xiezhu");
                break;
            case 4:
                label.text = TextManager.GetUIString("shuxing");
                break;
        }
    }
    ...
    //查看玩具信息
    public static void RoleShowInfo(int roleID)
    {
        if (roleID == PlayerMediator.playerInfo.roleId)
            Facade.Instance.SendNotification(NotificationID.Role_Show);
        else
            ServerCustom.instance.SendClientMethods("onClientGetPlayerInfo", roleID);
    }
